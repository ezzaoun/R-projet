...
```{r}
ggplot(Moy_Rep, aes(x = T2_6h, y = T3_6h, color = "T2_6h")) +
  geom_point() +
  geom_smooth(method = "lm", se = FALSE, color = "red") +
  labs(title = "Relation entre Y_T2_6h et Y_T3_6h", x = "Y_T2_6h", y = "Y_T3_6h") +
  theme_minimal()

ggplot(Moy_Rep, aes(x = T1_3h, y = T3_6h, color = "T1_3h")) +
  geom_point() +
  geom_smooth(method = "lm", se = FALSE, color = "blue") +
  labs(title = "Relation entre Y_T1_3h et Y_T3_6h", x = "Y_T1_3h", y = "Y_T3_6h") +
  theme_minimal()
```
---
...
# T1_1h+T1_2h+T1_3h+T2_1h+T2_2h+T2_3h 
...
# Sélection des prédicteurs : moyennes pour T1 et T2 aux heures 1h, 2h, et 3h
predictors <- filtered_data[, c("T1_1h", "T1_2h", "T1_3h", "T2_1h", "T2_2h", "T2_3h")]
target <- filtered_data$target

# Diviser les données en jeu d'entraînement et de test
set.seed(123) # Pour reproductibilité
trainIndex <- createDataPartition(target, p = 0.7, list = FALSE) # 70% des données pour le train
trainData <- predictors[trainIndex, ]
trainLabel <- target[trainIndex]
testData <- predictors[-trainIndex, ]
testLabel <- target[-trainIndex]

# Étape 2 : Ajustement du modèle (Régression logistique)
# Ajuster un modèle de régression logistique
model <- glm(trainLabel ~ ., data = data.frame(trainData, trainLabel), family = "binomial")

# Résumé du modèle
summary(model)

# Étape 3 : Prédiction sur le jeu de test
predictions <- predict(model, newdata = data.frame(testData), type = "response")
...
conf_matrix <- confusionMatrix(as.factor(predicted_class), as.factor(testLabel))
print(conf_matrix)

# Courbe ROC
roc_curve <- roc(testLabel, predictions)
