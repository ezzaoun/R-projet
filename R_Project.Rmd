...
# Analyse descriptivre du jeu de données
...
Tout d'abord, nous utiliserons la méthode \texttt{k-means} pour étudier le clustering des individus. Pour cet algorithme, on cherche à minimiser l'\texttt{inertie intraclasse}, qui est définie par : $I_{\text{inter}} = \sum_{k=1}^K |C_k| \times d(m_k, c)^2 \quad \text{où} \quad m_k = \frac{1}{|C_k|} \sum_{i \in C_k} x_i \text{ est le centre de gravité de la classe } C_k$.
...
Dans cette partie, nous allons mettre en place une classification hiérarchique ascendante. La méthode utilisée pour calculer la distance entre les gènes est la distance \texttt{k-means} définie par : $d(x_i, x_\ell) = \|x_i - x_\ell\|_2 = \sqrt{\sum_{j=1}^p (x_{ij} - x_{\ell j})^2}$.
Pour l'agrégation des clusters, nous optons pour la mesure d'agrégation \texttt{Le lien complet}, donnée par : $D(C_k, C_{k'}) = \frac{|C_k| \, |C_{k'}|}{|C_k| + |C_{k'}|} \, d(m_k, m_{k'})^2 \quad \text{où} \quad m_k \, (\text{resp. } m_{k'}) \text{ est le centre de gravité de } C_k \, (\text{resp. } C_{k'}) \text{ et } d \text{ est une distance euclidienne.}$.
Pour déterminer le nombre de clusters, nous introduisons le critère \texttt{Calinski-Harabasz} donné par : $\quad \text{PseudoF}(K) = \frac{I_{\text{inter}}(P_K)}{K - 1} \Big/ \frac{I_{\text{intra}}(P_K)}{n - K}$.
...
Nous nous intéressons ici à la méthode des mélanges gaussiens, basée sur l'hypothèse que les gènes suivent une distribution de probabilité composée de plusieurs distributions gaussiennes : $f(.|\theta_K) = \sum_{k=1}^K \pi_k f_k(.|\alpha_k)$. Pour sélectionner le bon modèle, nous optons pour le critère de sélection \texttt{ICL}, défini par : $\text{crit}(K) = L(x|\hat{\theta}_K) - \frac{\nu_K}{2} \ln(n) - \text{Ent}(K)$. Comme on sait que l'expression des gènes est dépendante, on peut d'ores et déjà retenir que les modèles permettant des degrés variés de liberté sont les plus adaptés. On en déduit que seuls les modèles suivants sont retenus (\texttt{modelNames}) :
...
Le modèle maximisant le critère \texttt{ICL} est "VEV", avec un nombre de clusters égal à 4. La figure \ref{fig:mmacp_ind} suivante représente les clusters obtenus des individus.


```{r, fig.pos='H', fig.align='center', fig.height=3, echo=FALSE, fig.cap="\\label{fig:mmacp_ind}Clusters représentés avec un Mélange Gaussien"}
...
Nous allons ici comparer les trois méthodes de clustering réalisées précédemment en utilisant un diagramme d'alluvions (figure \ref{fig:alluvion_ind}).

```{r, fig.pos='H', fig.align='center', fig.height=3, fig.cap="\\label{fig:alluvion_ind}Diagramme d'alluvions", echo=FALSE}
clust1 <- paste("K-k", reskmeans$cluster, sep = "")
...
ggplot(tmp, aes(x = Predicted, y = T3_6h)) +
  ...
  theme_minimal()
```
## Prédiction des gènes sur-exprimés et sous-exprimés à partir des observations pour les traitements T1 et T2 et les heures 1 à 3
...
```{r Roc,echo=F,fig.width=7,fig.height=4,fig.align='center',fig.cap="\\label{fig:Roc}courbe Roc",fig.pos='H'}
l<-1000
