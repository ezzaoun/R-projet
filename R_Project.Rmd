...
La Figure \ref{fig:Correlation_inter} montre une corrélation marquée entre les traitements 2 et 3, suggérant une expression similaire. En revanche, le traitement 1 présente une faible corrélation avec les traitements 2 et 3, indiquant une différence d’expression. Nous extrayons également la corrélation en fonction des réplicats pour approfondir cette analyse.

```{r Correlation_replicat, eval = T, echo = F,fig.width=2, fig.height = 2, fig.show='hold', fig.align = "center", fig.cap = "\\label{fig:CorrelationRep}Corrélation en fonction des réplicats",fig.pos='H'}
...
```{r Violin_plot, eval = T, echo = F, fig.width=5,fig.height=4, fig.align = "center", fig.cap = "\\label{fig:violin}Distribution des Expressions par Réplicat, Temps et Traitement",fig.pos='H'}
...
```{r lien,echo=F,fig.width=4, fig.height=3,fig.align='center',fig.cap="\\label{fig:lien_mel_kmeans}Comparaison entre les classifications Kmeans et MEL",fig.pos='H'}
...
Tout d'abord, nous utiliserons la méthode \texttt{k-means} pour étudier le clustering des individus. Pour cet algorithme, on cherche à minimiser l'\texttt{inertie intraclasse}, qui est définie par : 
$$
...
$$
Pour faire cette étude, nous devons avant tout déterminer le nombre de clusters \( K \). Pour cela, nous optons pour le choix du critère \texttt{silhouette} permettant d'évaluer la qualité du clustering en mesurant la cohésion intra-classe et la séparation entre les classes.
Le nombre optimal de clusters \( \hat{K} \) est celui qui maximise \( S(K) \).
...
```{r courbe_inter,echo=F,fig.width=5, fig.height=4,fig.align='center',fig.cap="\\label{fig:interaction}Interaction plot",fig.pos='H'}
...
```{r observed_fitted_plot, echo=F,fig.width=7, fig.height=4,fig.align='center',fig.cap="\\label{fig:obsVSpred1}Repésentation de la qualité du modèle",fig.pos='H'}
...
```{r observed_fitted,echo=F,fig.width=7,fig.height=4,fig.align='center',fig.cap="\\label{fig:obsVSpred2}Repésentation de la qualité du modèle",fig.pos='H'}
# Prédictions à partir du modèle
...
On va réaliser une regression linéaire pour prédire les gènes du traitement 3 à 6h en fonction des gènes des autres traitements à toutes les heures.  

```{r MLG, eval = T, echo=T, fig.cap="Modèle de regression linéaire complet"}
...
On remarque dans cette sortie, que la p-valeur pour le modèle constant est < 2.2e-16; on rejette le modèle constant. On se demande maintenant quelle est la meilleur modèlisation de ce modèle. Pour selectionner le modèle parcimonieux, on décide d'utiliser le critère de Mallows avec un méthode backward. On cherche à minimiser ce critère.

```{r Selection, fig.width=5, fig.height=4, echo=F, fig.show='hold', fig.align = "center", fig.cap = "\\label{fig:critereMallows}Critère de Mallows"}
...
Sur la Figure \ref{fig:critereMallows}, on remarque le critère $c_p$ selectionne toutes les variables sauf T1_5h et T2_1h. On décide de tester ce sous-modèle en le comparant au modèle de regression linéaire complété implémenté précédemment. 
...
Pour l'analyse de variable entre les 2 modèles, on obtient une p-valeur = 0.8284 > 0.05, on accepte donc largement le nouveau modèle. 

On souhaîte maintenant vérifier si notre nouveau modèle permet de bien exprimer les valeurs observées (gènes).


```{r predict, echo = F, fig.width=3,fig.height=2, fig.cap="\\label{fig:comp_vop}Comparaison valeurs observées et prédites"}
tmp = Moy_Rep
...
Sur la figure \ref{fig:comp_vop}, on compare une fonction affine de pente 1 et d'origine nulle aux valeurs observées en fonction des valeurs prédites. Cela permet de vérifier si on a bien prédit les valeurs; plus les points sont alignés à la droit, mieux est notre prédictions. On remarque ici que les points sont proches de cette droite, on peut donc en conclure que notre modèle à bien prédit les valeurs. 
...
Nous avons tenter d'exprimer les gènes sur-exprimés (codés 1) et les gènes sous-exprimés (codés 0) à 6h pour le traitement T3 à partir des observations pour les traitements T1 et T2 et les heures 1 à 3 pour ces mêmes gènes. Pour cela, on va réaliser un modèle linéaire généralisé car les variables que l'on souhaîte prédire sont de type binaire; on choisit ainsi une fonction de lien logit pour effectuer une regression logistique. 

Dans un premier temps, on prédit les gènes sur-exprimés et sous-esprimés et on affiche le nombre de gène identifié comme sous-exprimée(codés 0) et sur-exprmiée (codés 1) : 
...
On essayer ensuite de générer ce résultat à partir du modèle complet de régression logistique.
...
On obtient ici une p-valeur = 0.785 > 0.05, on accepte donc le modèle. 

```{r Roc,echo=F, eval=F,fig.width=7,fig.height=4,fig.align='center',fig.cap="\\label{fig:Roc}courbe Roc"}
l<-1000
...
```{r Roc2,echo=F,eval=F,fig.width=7,fig.height=4,fig.align='center',fig.cap="\\label{fig:Roc2}courbe Roc"}
