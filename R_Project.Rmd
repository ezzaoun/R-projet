...
---
On transpose les données pour pouvoir réaliser l’analyse des variables. 
---
...
adjustedRandIndex(modICL$classification,reskmeans$cluster)
table(modICL$classification,reskmeans$cluster)

clust1<-paste("MEL-k",modICL$classification,sep="")
clust2<-paste("K-k",reskmeans$cluster,sep="")
...
- Le temps : une variable quantitative.
- Le traitement : une variable qualitative.
...
$$
Y_{tsg} = \beta_0 + \beta_1 \cdot Temps + \beta_2 \cdot Traitement + \beta_3 \cdot (Temps \times Traitement) + \epsilon_{tsg} ,\ t\in\{1,\ldots,3\} ,\ s\in\{1,\ldots,6\} ,\ g\in\{1,\ldots,2144\}.
$$
...
Y_{tsg} = Y_{tsgR1} - Y_{tsgR2} 
\\ 
t\in\{1,\ldots,3\} ,\ s\in\{1,\ldots,6\} ,\ g\in\{1,\ldots,2144\}.
$$
...
```{r courbe_inter,echo=F,fig.width=5, fig.height=4}
...
```{r observed_fitted_plot, echo=F,fig.width=5, fig.height=3}
...
```{r Residuals_6h_1h,echo=F,fig.width=5, fig.height=3}
fitted_vals <- fitted(M_1h_to_6h)
residuals <- resid(M_1h_to_6h)
ggplot(data = data.frame(Fitted = fitted_vals, Residuals = residuals), aes(x = Fitted, y = Residuals)) +
...
Le modèle ajusté montre une qualité d'ajustement modérée, avec un R² de 27.06%. Les termes d’interaction entre Y1h et le traitement sont hautement significatifs (p<2e−16), ce qui montre que l’effet de $Y_{1h}$ sur $Y_{6h}$ varie selon le traitement. 
...
```{r observed_fitted,echo=F,fig.width=7,fig.height=4}
# Prédictions à partir du modèle
