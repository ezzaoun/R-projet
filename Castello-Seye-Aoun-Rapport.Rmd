...
Nous nous intéressons ici aux individus. La figure \ref{fig:acp_ind} représente ces individus en fonction des variables dans un espace réduit qui résume le mieux les données.
...
$$ 
Pour l'agrégation des clusters, nous optons pour la \texttt{mesure d’agrégation de Ward}, donnée par : 
$$
D(C_k, C_{k'}) = \frac{|C_k| \, |C_{k'}|}{|C_k| + |C_{k'}|} \, d(m_k, m_{k'})^2
\quad \text{où} \quad m_k \, (\text{resp. } m_{k'}) \text{ est le centre de gravité de } 
C_k \, (\text{resp. } C_{k'})
$$
...
```{r, fig.pos='H', fig.align='center', fig.cap="\\label{fig:deno_ind}Evolution du critère CH et Dendrogramme", fig.height=3, echo=FALSE}
library(ggdendro)
hclust_res <- hclust(dist(Data, method = "euclidean"), method = "ward.D2")
...
## Mélange gaussien
Étant donné que l'expression des gènes varie en fonction du traitement et du temps, il est pertinent d'opter pour des modèles offrant des degrés de liberté variés. Ainsi, seuls les modèles suivants ont été retenus (\texttt{modelNames}) :
...
Nous allons ici comparer les trois méthodes de clustering réalisées précédemment en utilisant un diagramme d'alluvions (figure \ref{fig:mmacp}).
...
La première observation marquante est le flux important partant du cluster 1 des \texttt{k-means} vers le cluster 1 de la \texttt{classification hiérarchique}, et se terminant dans les clusters 1, 2 et 4 du \texttt{mélange gaussien}. Les classes obtenues via la \texttt{classification hiérarchique} sont une agrégation des classes de \texttt{k-means}. On en déduit qu'elles sont concordantes.  


### MCA sur les différentes méthodes :

Une autre méthode de comparaison consiste à créer un tableau disjonctif complet qui présente, pour chaque gène, les clusters obtenus selon les trois méthodes de clustering : \texttt{k-means}, \texttt{classification hiérarchique} et \texttt{mélanges gaussiens}.
...
La première dimension de la figure \ref{fig:mca_ind} est associée aux trois méthodes, tandis que la deuxième dimension semble être principalement influencée par \texttt{k-means} et le \texttt{mélange gaussien}. Les proximités observées entre les modalités sur le plan révèlent une forte association entre elles. Par exemple, on constate que le cluster 1 obtenu par \texttt{k-means} et le cluster 1 de la méthode \texttt{hiérarchique} sont très proches, ce qui suggère que plusieurs gènes classés dans le cluster 1 de \texttt{k-means} se retrouvent également dans le cluster 1 de la méthode \texttt{hiérarchique}. Cette observation confirme la comparaison réalisée à l'aide du diagramme d'alluvions.
...
```{r, fig.pos='H',include=F, fig.align='center', echo=FALSE}
ks_test_result <- ks.test(replicat1, replicat2)
ks_test_result
```
Rejet de H0 : Les lois de probabilité des deux réplicats sont significativement différentes (p-value = 1.111068e-32).(*)
...
```{r Compare_modeles,include=F, echo=F}
...
```{r Reg, echo = F ,include=F, fig.cap="Modèle de regression linéaire complet"}
...
```{r Selection2, echo=F,include=F, fig.show='hold', fig.align = "center", fig.cap = " "}
